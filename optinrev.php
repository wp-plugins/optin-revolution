...
$optinrev_installed_version = '1.1.5';
...
function optinrev_wphead() {
  ...
  echo '<script type="text/javascript">/* <![CDATA[ */var ms = "", el = document.createElement(\'div\'), ch = jQuery(window).height(), exh = 30, c = jQuery(el).html(\''. $content .'\'), tshow = "'. $optinrev_play .'", isvalid = '.((isset($optin['validate']))?json_encode($optin['validate']):'{}').', mail_form_name = \''.$optin['optinrev_email_form_opt'].'\',optinrev_close_button_class = \''. $optin['optinrev_close_button_class'].'\', optinrev_top_margin = '.$optin['optinrev_top_margin'].',optinrev_wbg_opacity = '.$optin['optinrev_wbg_opacity'].', modal_delay = '.$modal_delay.',box_delay = box_started = 0, rnd='.$round_corner.',optinrev_visited_once='.$optinrev_visited_once.';/* ]]> */</script>';
}
