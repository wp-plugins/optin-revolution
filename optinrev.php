...
function optinrev_js()
{
  ...
  wp_enqueue_script( 'optinrevjsv', site_url( sprintf('?optinrev-edit-init-js=%s&t=', $plugin_page, optinrev_cid()) ), array(), $wp_version, true );
...
function optinrev_wphead() {
  ...
  $js = sprintf('<link rel="stylesheet" href="'.site_url( '?optinrev-popup=' . $optin_id ).'&view=%s" type="text/css">', optinrev_cid());
  $js .= sprintf('<script type="text/javascript" src="'. site_url( '?optinrev-popup-vars=' . $optin_id ) .'&t=%s"></script>', optinrev_cid());
  ...
  add_filter('query_vars','optinrev_plugin_add_trigger');
  function optinrev_plugin_add_trigger($vars) {
      array_push( $vars, 'optinrev-popup', 'optinrev-popup-vars', 'optinrev-wysija', 'optinrev-ccform', 'optinrev-edit-init-js' );
      return $vars;
  }
