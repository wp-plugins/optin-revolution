  ...
  this.applyCorners = function() {
    ...
    */
    ...
    if (topMaxRadius) {
      ...
      //knight @ 2010-08-18 begin
      //newMainContainer.style.width = this.boxWidth + "px";
      newMainContainer.style.width = this.boxWidth + this.borderWidthL + this.borderWidthR - 2*this.borderWidth + "px";
      //knight @ 2010-08-18 end
    ...
    if (botMaxRadius) {
      ...
      //knight @ 2010-08-18 begin
      //newMainContainer.style.width = this.boxWidth + "px";
      newMainContainer.style.width = this.boxWidth + this.borderWidthL + this.borderWidthR - 2*this.borderWidthB + "px";
      //knight @ 2010-08-18 end
      newMainContainer.style.fontSize = "1px";
      newMainContainer.style.overflow = "hidden";
      newMainContainer.style.position = "absolute";
    ...
    */
    for (var i in corners) if (!isNaN(i)) {
      ...
      var bcolorTB, bcolorLR, bwidthDiff//knight @ 2010-08-18
      if (cc == "tr" || cc == "tl") {
        bwidth = this.borderWidth;
        bcolor = this.borderColour;
        borderWidthTB = this.borderWidth;
        //knight @ 2010-08-18 begin
        bcolorTB = this.borderColour;
        bcolorLR = cc == "tr"? this.borderColourR : this.borderColourL;
        bwidthDiff = cc == "tr"? this.borderWidth - this.borderWidthR : this.borderWidth - this.borderWidthL; //this maybe negative value..
        //knight @ 2010-08-18 end
      } else {
        ...
        //knight @ 2010-08-18 begin
        bcolorTB = this.borderColourB;
        bcolorLR = cc == "br"? this.borderColourR : this.borderColourL;
        bwidthDiff = cc == "br"? this.borderWidthB - this.borderWidthR : this.borderWidthB - this.borderWidthL; //this maybe negative value..
        //knight @ 2010-08-18 end
      }
      ...
      for (intx = 0; intx < specRadius; ++intx) {
        ...
        //to insure the accurate, put the square in the bracket.
        var y1 = (intx + 1 >= borderRadius + bwidthDiff) ? -1 :Math.floor(Math.sqrt(Math.pow(borderRadius,2) - Math.pow(intx +1,2)*Math.pow(borderRadius,2)/Math.pow(borderRadius + bwidthDiff,2))) -1;
        //knight @ 2010-08-18 end
        ...
        if (borderRadius != specRadius) {
          ...
          //to insure the accurate, put the square in the bracket.
          var y2 = (intx >= borderRadius + bwidthDiff) ? -1 : Math.ceil(Math.sqrt(Math.pow(borderRadius,2) - Math.pow(intx,2)*Math.pow(borderRadius,2)/Math.pow(borderRadius + bwidthDiff,2)));
          //knight @ 2010-08-18 end
          ...
          if (this.spec.antiAlias) {
            for (inty = y1 + 1; inty < y2; ++inty) {
              ...
              if (this.backgroundImage != "") {
                ...
                var borderFract = curvyObject.pixelFractionNew(intx, inty, borderRadius+bwidthDiff,borderRadius) * 100;
	              if(intx <= Math.ceil(specRadius/2))
	                this.drawPixel(intx,inty,bcolorTB,trans,1,newCorner,borderFract>=30,specRadius);
	              else
	                this.drawPixel(intx,inty,bcolorLR,trans,1,newCorner,borderFract>=30,specRadius);
                //knight @ 2010-08-18 end
              }
                ...
                var pixelcolor;
	              if(intx <= Math.ceil(specRadius/2))
	                pixelcolor = curvyObject.BlendColour(this.boxColour, bcolorTB, curvyObject.pixelFractionNew(intx, inty, borderRadius + bwidthDiff,borderRadius)); 
	              else
	                pixelcolor = curvyObject.BlendColour(this.boxColour, bcolorLR, curvyObject.pixelFractionNew(intx, inty, borderRadius + bwidthDiff,borderRadius)); 
            ...
            if (y3 >= y2) {
              ...
              if(intx <= Math.ceil(specRadius/2))
	              this.drawPixel(intx, y2, bcolorTB, trans, (y3 - y2 +1), newCorner, false, 0);
	            else
	              this.drawPixel(intx, y2, bcolorLR, trans, (y3 - y2 +1), newCorner, false, 0);
              //knight @ 2010-08-18 end
            }
            //knight @ 2010-08-18 end
            //outsideColour = bcolor;  // Set the colour for the outside AA curve
            if (intx <= Math.ceil(specRadius/2))
	            outsideColour = bcolorTB;       //set the color for the outside
	          else
	            outsideColour = bcolorLR;
	          //knight @ 2010-08-18 end
        ...
        if (this.spec.antiAlias && this.boxColour !== 'transparent') {
          ...
          while (++inty < y4) {
            ...
            this.drawPixel(intx, inty, outsideColour, (curvyObject.pixelFractionNew(intx, inty , specRadius,specRadius) * trans), 1, newCorner, borderWidthTB <= 0, specRadius);
          }
        }
      }
    ...
    */
      ...
      switch (z) {
        case "t":
          ...
          if (this.topContainer) {
            ...
            newFillerBar.style.marginLeft  = this.spec.tlR ? (this.spec.tlR -this.borderWidth) + "px" : "0";
            //knight @ 2010-08-18 end
        ...
        case "b":
          if (this.bottomContainer) {
            ...
            newFillerBar.style.marginLeft   = this.spec.blR ? (this.spec.blR - this.borderWidthB) + "px" : "0";
            //knight @ 2010-08-18 end
    ...
    this.contentContainer.id = "curvy2";
...
//New fraction method to satisfied the different border width box

curvyObject.pixelFractionNew = function(x,y,a,b){
  var fraction;
  //var rsquared = r*r;
  /*
    determine the co-ordinates of the two points on the perimeter of the pixel that the
  ...
  */
  ...
  //x+0 = left
  //var intersect = Math.sqrt(rsquared - Math.pow(x,2));
  var intersect = Math.sqrt(Math.pow(b,2) - Math.pow(x,2)*Math.pow(b,2)/Math.pow(a,2)); //to insure the accurate, put the square in the bracket.
  if(intersect >= y && intersect < (y+1)){
   whatsides = "Left";
   xvalues[point] = 0;
   yvalues[point] = intersect - y;
  ...
  // y +1 = top
  //intersect = Math.sqrt(rsquared - Math.pow(y+1,2));
  intersect = Math.sqrt(Math.pow(a,2) - Math.pow(y+1,2)*Math.pow(a,2)/Math.pow(b,2));
  if (intersect >= x && intersect < (x + 1)) {
    whatsides += "Top";
    xvalues[point] = intersect - x;
    yvalues[point] = 1;
  ...
  // x + 1 = Right
  //intersect = Math.sqrt(rsquared - Math.pow(x + 1, 2));
  intersect = Math.sqrt(Math.pow(b,2) - Math.pow(x+1,2)*Math.pow(b,2)/Math.pow(a,2));
  if (intersect >= y && intersect < (y + 1)) {
    whatsides += "Right";
    xvalues[point] = 1;
    yvalues[point] = intersect - y;
    ...
    // y + 0 = Bottom
  //intersect = Math.sqrt(rsquared - Math.pow(y, 2));
  intersect = Math.sqrt(Math.pow(a,2) - Math.pow(y,2)*Math.pow(a,2)/Math.pow(b,2));
  if (intersect >= x && intersect < (x + 1)) {
    whatsides += "Bottom";
    xvalues[point] = intersect - x;
    yvalues[point] = 0;
