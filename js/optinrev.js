...
jQuery(document).ready( function($) {
    ...
    inputfields_update : function(){
        ...
        var inputs = mpr.input, inputs = inputs.split(','), ilen = Math.ceil(inputs.length / 2), hrec = 0;
        var boxin = $('#wotinput_fields');
        
        imn = 1;
        
        //finding fields        
        for( i = 0; i < inputs.length; i++ ) {
            if ( /first_name|last_name|name\(awf_first\)|name\(awf_last\)|FNAME|LNAME|fields_fname|fields_lname|fname|lname/g.test( inputs[i] ) == false )         
            if ( $('input[data-id="'+ inputs[i] +'"]', boxin ).val() == '' ) {
            hrec++;
            }
        }
