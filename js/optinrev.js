  ...
  redraw: function()
  {
   var r, wtdom = tinyMCE.activeEditor.dom, rnd = parseInt( j('#optinrev_border_radius').val() ), sc = wtdom.get('simplemodal-container');

    //fix broken asset images
    j('img', sc).each(function(i, img){
        var im = j(img).attr('src');
        if ( im.indexOf('../wp-content/plugins/optin-revolution/') !== -1 ) {
        j(img, sc).attr('src', im.replace(/\.\.\/wp-content\/plugins\/optin\-revolution\//, wtp));
        }
    });
   ...
   j('a', sc).click(function(e){ e.preventDefault();return false;});
 ...
 set_optin_default: function( msg ) {
  jQuery.post('admin-ajax.php', {action : 'optinrev_action', optinrev_popup_reset : 77}, function(res){
 ...
 tinymce: function( ed )
 {
          ...
          if ( t.isIE ) {
                  ...
                  if ( typeof n.style.direction !== 'undefined' ) {
                          if ( n.style.direction == 'ltr' ) {
                                ...
                                jQuery(n).bind("oncontrolselect", function(){ return false; });
                                jQuery(n).bind("onmousedown", function(e){ e.returnValue = false; this.hideFocus = true; });
                                jQuery(n).bind("onselect", function(){ this.hideFocus = true; return false; });
                                jQuery(n).bind("onclick", function(){ return false; });
                                jQuery(n).bind("ondrag", function(){ return false; });
                          }
                      }
                  }
        ...
        tinymce.dom.Event.add( v, 'mousedown', function(e)
        {
          ...
          if ( el = ed.dom.getParent( e.target, 'DIV' ) )
          {
            ...
            if ( tinymce.isIE )
            {
              ...
              jQuery(el).bind("oncontrolselect", function(){ return false; });
            }
   ...
   ed.onInit.add( function( ed )
   {
     ...
     if ( tinymce.isIE )
     {
      ...
      jQuery(bwn).bind("onmousedown", function(){ return false; });
      jQuery(bwn).bind("onselect", function(){ return false; });
      jQuery(bwn).bind("onclick", function(){ return false; });
      jQuery(bwn).bind("ondrag", function(){ return false; });
      jQuery(bwn).bind("oncontrolselect", function(){ return false; });
    }
    ...
    j('a', bwn).click(function(e){ e.preventDefault();return false;});
 ...
 close_button_pos : function( btn_class ) {
  ...
  optin_form_default: function() {
    if ( typeof tinyMCE.activeEditor != 'undefined' )
    {
      ...
      jQuery( txtfld ).css({ 'position': 'absolute', 'left' : loc.left, 'top': loc.top, 'z-index': 5, 'border': '1px solid transparent' }).append( infld );
  ...
  if ( fcode === 'constantcontact' ) {
     if ( typeof jQuery('#cwait')[0] !== undefined ) {
  ...
  if ( fcode == 'aweber'  ) {
      ...
      if ( jQuery('#optinrev_ctcurl').val() === '' ) {
          alert('Please enter the share url');
          jQuery('#optinrev_ctcurl').focus();
          return false;
      }
  ...
  //if ( typeof wtdom.select("form")[0] !== 'undefined' )
  //wtdom.remove(wtdom.select('form')[0]);
  jQuery( 'form', wtdom.get( 'simplemodal-data' ) ).remove();
  ...
  jQuery.post( doc_base_url +"?optinrev-ccform=1", {url: u}, function(res){
  jQuery('#cwait').remove();
  ...
  jQuery.post( site_url+"?optinrev-ccform=1", {url: u}, function(res){
  ...
  jQuery('ul', mn).each(function(i, el){
...
jQuery(document).ready( function($) {
    ...
    //wotinput fields
    setTimeout(function(){
      $('#wotinput_fields').load('admin-ajax.php', {action : 'optinrev_action', optinrev_mail_webform : wtpage, optinrev_mail_provider: $('input[name="optinrev_email_form_opt"]:checked').val()}, function(){
