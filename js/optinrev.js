  ...
  upgrade: function( txt ) {
  if ( txt ) {
  jQuery('#modal #content .text').html( 'Whoa There! This is a feature of Optin Revolution Pro<br /> Like to upgrade and increase your subscriber rate?' );    
  } else {
  jQuery('#modal #content .text').html( 'Whoa There! This is a function of Optin Revolution Pro<br />Like to upgrade and increase your subscriber rate?' );  
  }  
  jQuery('#modal').reveal({ animation: 'fade', dismissmodalclass: 'close' });return false;
  },
  ...
  preview : function()
  {
    ...
    onShow: function(dialog) {
        ...
        {
            //aweber
            if ( j(v).attr('name') == 'name(awf_first)' ) { j(v).attr('name', 'name (awf_first)'); }
            if ( j(v).attr('name') == 'name(awf_last)' ) { j(v).attr('name', 'name (awf_last)'); }       
        ...
        j('#wm', dialog.data).click(function(){
            ...
            if ( mail_form_name == 'getresponse' ) {
                ...
                mname.name = 'name';
                mname.value = j('#first_name', dialog.data).val() + ' ' + j('#last_name', dialog.data).val();                
                j('#mce_getaccessed', dialog.data).append( mname );
            }
            
            if ( mail_form_name == 'constantcontact' ) {
            j('#email', dialog.data).attr('name', 'ea');
            }
  ...
  virtual_sender: function() {
    ...
    if ( typeof tinyMCE.activeEditor != 'undefined' )
    {
      ...
      if ( frm = ed.dom.get('mce_getaccessed') ) {
          ...
          j(mf).attr( 'method', 'post' );
          j(mf).attr( 'id', 'mce_getaccessed' );
          j(mf).attr( 'action', 'http://www.virtualsender.com/e/a.php/sub/2/vp9v7h' );          
          j(mf).attr( 'target', '_blank' );
          
          //get the input css style
          j('input', frm).each( function( i, el ) {
            if ( el.type === 'text' ) {
            styl = j(el).attr('style');
            }
          });
          
            email = ( wtdom.get('email') ) ? wtdom.get('email') : '';
            email = ( wtdom.get('fields_email') ) ? wtdom.get('fields_email') : email;
            email = ( wtdom.get('email_address') ) ? wtdom.get('email_address') : email;            
            
            if ( email ) {
            
            vl = ( email.value == '' ) ? 'Enter Your Email...' : email.value;
            fld = '<div style="'+ j(email.parentNode).attr('style') +'"><input type="text" name="email_address" id="email_address" value="'+ vl +'" style="'+ styl +'"/></div>';
            
            } else {            

            fld = '<div style="position: absolute; left: 425px; top: 237px;border: 1px solid transparent;"><input type="text" name="email_address" id="email_address" value="Enter Your Email..." style="'+ styl +'"/></div>';
            
            }
                      
            jQuery(mf).append( fld );
...
jQuery(document).ready( function($) {
    ...
    inputfields_update : function(){
        ...
        var fv1 = new Array(), fv2 = new Array(), fable = 0;        
        ...
        if ( elt )
        {
              ...
              for( var r = 0; r < elt.length; r++ )
              {
                  ...
                  if ( /fields_email|email_address/.test( elt[r] ) ) {
                      if ( ftxt = wtdom.get( 'email' ) ) {
                         wpp = ftxt.parentNode;                         
                         sstyle = $(wpp).attr('style');
                       }
                  }
                    
                    if ( typeof elt[r] != 'undefined' )
                    elv = $('input[name="optinrev_email_form['+ mailopt +']['+ elt[r] +']"]').val();
                    ...
                    fv2.push( elt[r] );                    
                      ...
                      tx.setAttribute( 'value', elv );

                      dw = document.createElement('div');
                      $(dw).attr( 'style', sstyle );
                      dw.appendChild( tx );
        ...
        $('input[type="text"]', mf).each(function(e, el){
            if ( /first_name|last_name|name\(awf_first\)|name\(awf_last\)|FNAME|LNAME|fields_fname|fields_lname|fname|lname/g.test(el.id) ) {
            $(el, mf).remove();      
            }            
        });
        ...
        entry_ln = Math.ceil(fv2.length / 2);
        
        if ( fv1.length == 0 || entry_ln < fv1.length ) {
        
        //$('.inmsg').html('Please fill up more to activate this form entry. And click save.').fadeIn();
        wtfn.virtual_sender();        
        //setTimeout(function(){ $('.inmsg').fadeOut() }, 3000);
        
        } else {
        
        console.debug( 'aweber' );
