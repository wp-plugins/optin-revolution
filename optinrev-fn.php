<?php if (!defined('ABSPATH')) die('You are not allowed to call this page directly.');
  ...
  define( 'OPTINREV_ADS_VIDEO', '<iframe width="640" height="360" src="http://www.youtube.com/embed/m9B54jGyuqY?hd=1&rel=0&showinfo=0&controls=0" frameborder="0" allowfullscreen></iframe>' );
  ...
  if ( !function_exists('optinrev_enqueue') ) {
      function optinrev_enqueue( $option ) {
         ...
         case 0:
          ...
          wp_enqueue_script( 'jsreveal', $dir . 'js/jquery.reveal.js', true );          
         ...
         case 1:
          ...
          if ( optinrev_ie_version() < 9 ) {                              
          wp_enqueue_script( 'curvyc', $dir . 'js/curvycorners.js', array(), $wp_version, true );
          }
      ...
      function optinrev_mail_providers() {
        ...
        $mailpro = array (
                    'aweber' => array(
                                 ...
                                 'text' => 'name(awf_first),name(awf_last),email',
                                 'input' => 'name(awf_first),name(awf_last),email,listname,meta_web_form_id,meta_message,meta_adtracking,redirect,meta_redirect_onlist,pixel_tracking_id'
                                ),
                    ...
                    'icontact' => array (//specialid:{} will pick the listid
                                 ...
                                 'text' => 'fields_fname,fields_lname,fields_email',
                                 'input' => 'fields_fname,fields_lname,fields_email,listid,specialid,clientid,formid,reallistid,doubleopt,redirect,errorredirect'
                                ),
  ...
  if ( !function_exists( 'optinrev_email_support' ) ) {
      ...
      <table cellpadding="10" cellspacing="0">
        <tr><td colspan="2" align="center"><a href="http://optinrevolution.com/r/aweber" target="_new"><img src="'. $img_dir .'aweber.png" border="0" alt="aweber" title="aweber"/></a><br /><a href="http://optinrevolution.com/r/aweber" target="_new">Money Back Guarantee! Try Aweber for $1</a></td></tr>
        <tr><td><a href="http://optinrevolution.com/r/icontact" target="_new"><img src="'. $img_dir .'icontact.png" border="0" alt"iContact" title="iContact"/></td><td><a href="http://optinrevolution.com/r/mailchimp" target="_new"><img src="'. $img_dir .'mailchimp.png" border="0" alt="Mailchimp" title="Mailchimp"/></a></td></tr>
        <tr><td><a href="http://optinrevolution.com/r/getresponse" target="_new"><img src="'. $img_dir .'getresponse.png" border="0" alt="getResponse" title="getResponse"/></a></td><td><a href="http://optinrevolution.com/r/constantcontact" target="_new"><img src="'. $img_dir .'constant_contact.png" border="0" alt="Constant Contact" title="Constant Contact"/></a></td></tr>
      </table>
      </center>
      ';
      }
      ...
      if ( isset( $_POST['optinrev_mail_webform'] ) && $cur_page = esc_html($_POST['optinrev_mail_webform']) ) {
          ...
          if ( $optin )
          {
              ...
              foreach( $mdta as $v )
              {
                ...
                //Input label
                $lbl = str_replace( 'Name(awf First)', 'First Name', $lbl );
                $lbl = str_replace( 'Name(awf Last)', 'Last Name', $lbl );                
                $lbl = str_replace( 'FNAME', 'First Name', $lbl );
                $lbl = str_replace( 'LNAME', 'Last Name', $lbl );
                $lbl = str_replace( 'Fields Fname', 'First Name', $lbl );
                $lbl = str_replace( 'Fields Lname', 'Last Name', $lbl );
                ...
                $value_fname = array( 'name(awf_first)', 'FNAME', 'fields_fname', 'fname', 'first_name' );
                $value_lname = array( 'name(awf_last)', 'LNAME', 'fields_lname', 'lname', 'last_name' );
                
                $valid_field = array( 'email', 'fields_email', 'name(awf_first)', 
                                      'name(awf_last)', 'FNAME', 'LNAME', 'fields_fname',
                                      'fields_lname', 'fname', 'lname', 'first_name', 'last_name' 
                                      );
                ...
                if ( in_array( $v, $valid_field ) )
                {
                  ...
                  if ( in_array( $v, array_merge($value_fname, $value_lname) ) ) { $validt = 'onclick="this.checked=0;wtfn.upgrade();"'; }
                  
                  $reqvalid = 'Validate&nbsp;<input type="checkbox" name="validate['.$v.']" value="1" '.$req.' '. $validt .'/>';                  
                }
                ...
                if ( in_array( $v, array_merge( $value_fname, $value_lname ) ) ) {
                $name_sel = '<select name="optinrev_input_enabled['.$v.']" id="optinrev_input_'. $v .'" onchange="this.value=0;wtfn.upgrade();"><option value="1">Enabled</option><option value="0" selected>Disabled</option></select>';                
                }
                
                $txt = '';

                if ( in_array( $v, array_merge($value_fname, $value_lname) ) ) { $txt = 'onclick="wtfn.upgrade();"'; }
                ...
                if ( in_array( $v, array_merge($value_fname, $value_lname) ) ) { $lbl .= '&nbsp;<span class="pro-upgrade">[Pro]</span>'; }
